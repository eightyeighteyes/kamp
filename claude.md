...
# Lessons Learned
## Syncer callbacks must be mirrored on the KampBandcampSyncer wrapper — KAMP-436
`core.syncer` is a `KampBandcampSyncer` (`kamp_daemon/ext/builtin/bandcamp.py`), a thin extension wrapper that delegates to an inner `Syncer` via **explicit `@property`/setter pairs** (`status_callback`, `error_callback`, `on_tracks_indexed`, `progress_callback`, …). When you add a new callback attribute to `Syncer`, you MUST also add a matching delegating property/setter to the wrapper — otherwise `core.syncer.<new_callback> = fn` in `__main__.py` sets a **dead attribute on the wrapper**, the inner syncer that actually runs the work never sees it, and the feature silently no-ops. KAMP-436 shipped byte-download progress that computed and broadcast correctly at every layer but never fired, because `progress_callback` had no wrapper setter (the daemon log showed the parent demuxing `progress:` with `callback=False`). Symptom to watch for: everything works in a direct-`Syncer` unit test but does nothing in the running daemon. When adding a `Syncer` callback, grep the wrapper for the sibling setters and add yours alongside.
...
- **Synthetic old-schema DBs: `DROP VIEW` before `ALTER TABLE … DROP COLUMN` — local pytest cannot catch this.** The dev venv is Python 3.13 (SQLite 3.51) but CI runs Python **3.11** (older bundled SQLite), and the two disagree on whether a `DROP COLUMN` may proceed when a *view* still references that column. `tracks_with_stats` projects every non-derived `tracks` column through, so any test that fakes a pre-migration DB via `ALTER TABLE tracks DROP COLUMN <col>` passes locally and fails **only on CI** with `sqlite3.OperationalError: error in view tracks_with_stats after drop column`. Always `DROP VIEW IF EXISTS tracks_with_stats` first — that is also the more faithful simulation, since a real pre-migration DB's view was built (from `PRAGMA table_info`) without the column, and `_create_tracks_with_stats_view` re-creates it after `_migrate` returns. Corollary: **any SQLite-behavior assumption is unverifiable locally** — the version gap between dev and CI is real, so prefer setups that don't depend on version-specific DDL semantics.
...
## The Bandcamp collection endpoint is the scarce resource — KAMP-637/639
...
- **A walk is not one request — it is `ceil(collection_size / _COLLECTION_PAGE_BATCH)` POSTs.** At the old batch of 20, an 800-album collection cost ~40 POSTs *per walk*, so "three walks in a minute" was ~120 POSTs at the endpoint that limits hardest. This was invisible for years because everyone counted walks, not requests. The live API honours `count` of 20/50/100/200 (returning exactly that many items *and* their matching `redownload_urls`), so the batch is 100. Before optimising anything else here, check what a "single call" actually costs.
- **Coalescing must never starve the head of the queue (KAMP-639).** Delaying a walk for a newly queued id is free only while that id is *waiting*. Once the drain is about to claim it, the skip hands it to the per-item fallback in `download_single_album`, which walks the whole collection *by itself* — so the window trades one prefetch walk for several fallback walks, the exact thing it exists to prevent. The quiet window may only delay a **speculative** walk; if `next_queued_download()` has no URL, walk now, because one walk resolves every queued row at once. The failure signature is a single "Prefetched" line followed by `collection_items` 429s minutes later.
- **A rate limit is account-wide, not the item's fault.** Failing the item and pulling the next one converts one 429 into a queue-wide cascade — each remaining item burns its own retries *and* its own collection walk. `process_next_download` raises `RateLimited`, returns the item to `queued` in place, and the worker sleeps (60s → 120s → 300s, reset by a clean drain). Never clear `redownload_url` on a 429 either: a 429 says nothing about the link, and dropping it forces the retry down the slow path against the endpoint already refusing us.
- **A pause that isn't visible is a hang.** The backoff shipped without any UI signal and was immediately reported as "downloads seem completely hung" — every row still read "queued" while nothing happened. The `download.queue` snapshot and `GET /api/v1/downloads` carry `paused_until` (Unix seconds, 0 when running) and the Downloads view counts down locally, so the daemon publishes the deadline once rather than broadcasting per second.

**Measuring bandcamp.com traffic on Windows:** Electron fetches `/api/v1/bandcamp/session-cookies` once per **cookie-lease acquisition** (one per burst of concurrent requests since KAMP-640, previously 1:1 per request), so it is a lower bound rather than an exact counter. For attribution, `_ProxySession._fetch` logs any relayed 4xx/5xx with the URL, status and body — that is what finally showed the 429s were on `collection_items` and then, once walks were coalesced, on the per-item download page. Useful baseline: a full stream sync runs at **every app start** and walked 773 albums for ~60 requests in about a minute — budget for that before blaming the download path for a 429.

## Concurrent relayed requests share ONE cookie jar — KAMP-640

The Electron proxy relay is the only path Bandcamp traffic takes on Windows and in every frozen build, and it is **concurrent**: `kampAPI.ts` fires `ipcRenderer.invoke` per WebSocket event *without awaiting*, and `ipcMain.handle` runs each async callback concurrently. Six producers (stream sync, download worker, size-backfill, two genre-enrich threads, playback lookahead) plus the download subprocess all feed it.

They share `session.defaultSession`. Injecting the cookies per request and deleting them afterwards therefore raced: whichever request finished first deleted them out from under everything still in flight, and those requests went out **unauthenticated** — which Bandcamp answers with 429. The symptom is a rate limit; the cause is lost authentication, so no amount of backing off helps.

- **This is the structural reason macOS is stable and Windows is not.** macOS dev never uses the relay: each thread has its own `requests.Session` with a private cookie jar, so there is no shared mutable state to race over. Any "works on macOS, fails on Windows" Bandcamp bug should be checked against the relay's shared state before anything else.
- The fix is a **reference-counted cookie lease**: first request in injects, concurrent requests await that same promise, removal happens only when the last one releases (in a `finally`, so a throw or timeout cannot leak a lease and pin cookies on disk). A fresh injection must chain behind any cleanup still draining, or that cleanup deletes the cookies the newcomer just wrote — the same race one level down.
- **Don't reach for a request-rate limiter for this class of bug.** Volume was essentially unchanged before and after (~43/min vs ~54/min peak); correctness was the whole problem.
