...
class TestFitPlaylistLabel:
    """Unit tests for _fit_playlist_label font-size / truncation logic."""

    def _fit(self, title: str):
        from kamp_core.server import _fit_playlist_label

        return _fit_playlist_label(title)

    def test_empty_title_uses_max_font(self) -> None:
        display, font = self._fit("")
        assert display == ""
        assert font == "5.5"

    def test_short_title_uses_max_font(self) -> None:
        _, font = self._fit("My Mix")
        assert font == "5.5"

    def test_eleven_char_title_uses_max_font(self) -> None:
        # n=11: ideal = (44/11 - 0.5)/0.6 = 5.83 → capped at 5.5
        _, font = self._fit("a" * 11)
        assert font == "5.5"

    def test_twelve_char_title_slightly_below_max(self) -> None:
        # n=12: ideal ≈ 5.28 → 5.3 (just under the cap)
        _, font = self._fit("a" * 12)
        assert float(font) < 5.5
        assert float(font) >= 5.0

    def test_longer_title_shrinks_font(self) -> None:
        _, font_12 = self._fit("a" * 12)
        _, font_16 = self._fit("a" * 16)
        assert float(font_16) < float(font_12)
        assert float(font_16) >= 3.5

    def test_font_never_below_minimum(self) -> None:
        for n in range(17, 50):
            _, font = self._fit("a" * n)
            assert float(font) >= 3.5

    def test_long_title_truncated_with_ellipsis(self) -> None:
        display, font = self._fit("a" * 30)
        assert display.endswith("…")
        assert font == "3.5"

    def test_truncated_display_fits_at_min_font(self) -> None:
        display, _ = self._fit("a" * 30)
        # At MIN_FONT=3.5, LABEL_WIDTH=44, advance per char ≈ 2.6 → max 16 display chars
